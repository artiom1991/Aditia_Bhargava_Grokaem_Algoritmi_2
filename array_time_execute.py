...
# Быстрее создать новый список из 10_000 элементов и в него уже добавить изначальный список из 1_000_000 элементов
# чем в начало списка из 1_000_000 значений добавить 10_000 элементов так как из-за специфики работы массива,
# ему нужно перезаписывать все значения, что может быть затратно.

start = time.time()
newArr = [i for i in range(1, 10_000)]
for i in arr:
    newArr.append(i)
end = time.time()
...
start = time.time()
newArr2 = [i for i in range(1, 10_000)] + arr
end = time.time()
print("Создалися список из 10_000 элементов  в который добавился список из 1_000_000", end - start)
